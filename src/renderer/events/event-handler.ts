...
const { updateAppVersionInfo } = toRefs(useVersionStore(pinia));
...
function getElectronAppConfig(result) {
  console.log(result, 'listener.ts::19行');
}

function electronUpdaterMessage(res) {
  ...
  if (key === 'downloadProgress') {
    msgTitle = '程序包下载中，可以点击隐藏按钮关掉弹窗，在下载成功后会弹出安装提示...';
  }
...
// 导出注册方法
export function injectGlobalMainEventListener() {
  ipcHelperUtil.addMainEventListener('consoleLog', consoleLog);
  ipcHelperUtil.addMainEventListener('showNotice', showNotice);
  ipcHelperUtil.addMainEventListener('getElectronAppConfig', getElectronAppConfig);
  ipcHelperUtil.addMainEventListener('electronUpdaterMessage', electronUpdaterMessage);
}
